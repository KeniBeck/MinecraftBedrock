...
import { useKickPlayer, useOnlinePlayers, useSearchPlayer, useBanPlayerOnServer } from './hooks'
...
export function PlayersPage() {
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Buscador de jugador por gamertag (resuelve XUID). */}
        ...
        <div className="flex items-center gap-2">
          <div className="relative flex-1">
            ...
            <Input
              ...
              placeholder="Gamertag de Xbox (ej. Notch)"
            />
          </div>
        </div>

        {query.trim() && search.isLoading && (
          <p className="mt-2 text-xs text-muted-foreground">Resolviendo…</p>
        )}
        {query.trim() && !search.isLoading && search.isError && (
          <p className="mt-2 text-xs text-red-300">
            {getApiCode(search.error) === 'PLAYER.NOT_FOUND'
              ...
              : getApiMessage(search.error, 'Error al resolver el jugador')}
          </p>
        )}
        {searchResult && (
          <div className="mt-3 flex items-center justify-between rounded-none border border-emerald-500/30 bg-emerald-500/10 px-3 py-2">
            <div className="min-w-0">
              <p className="truncate text-sm font-medium">{searchResult.name}</p>
              <p className="text-xs text-muted-foreground">XUID: {searchResult.xuid}</p>
            </div>
            {(canManage || canWrite) && (
              <div className="flex gap-2">
                {canManage && (
                  <Button
                    ...
                    size="sm"
                    pixel
                    onClick={() => setKickTarget(searchResult.xuid)}
                  >
                    Kick
                  </Button>
                )}
                {canWrite && (
                  <Button
                    ...
                    size="sm"
                    pixel
                    onClick={() => setBanTarget(searchResult.xuid)}
                  >
                    Ban
                  </Button>
                )}
